                    ...
                    <div className="flex space-x-1 sm:space-x-2">
                      {exp.logos.map((logo, i) => (
                        <div key={logo} className="w-8 h-8 sm:w-12 sm:h-12 bg-white border border-slate-200 shadow flex items-center justify-center overflow-hidden">
                          <Image src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}${logo}`} alt={`${exp.company} Logo ${i+1}`} width={32} height={32} className="object-contain max-w-full max-h-full" />
                        </div>
                      ))}
                    </div>
                  ) : (
                    <Image src={`${process.env.NEXT_PUBLIC_BASE_PATH || ''}${exp.logo}`} alt={`${exp.company} Logo`} width={32} height={32} className="object-contain flex-shrink-0" />
                  )}
